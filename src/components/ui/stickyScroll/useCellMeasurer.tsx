 ...
 */
export default function useCellMeasurer(items: ReactNode[]) {
  const root = useAppDispatch();
  ...
  const [hiddenSizingEl, setHiddenSizingEl] = useState<Element>();
  ...
  const itemSize = useCallback(
    (index: any) => {
      ...
      hiddenSizingEl.innerHTML = renderToString(
        <Provider store={store}>{item}</Provider>
      );
  ...
  useEffect(() => {
    ...
    document.getElementById("App")?.appendChild(newHiddenSizingEl);
  }, [hiddenSizingEl, id]);
  ...
  useEffect(() => {
    ...
    return () => {
      ...
      if (hiddenSizingElement)
        document.getElementById("App")?.removeChild(hiddenSizingElement);
    };
  }, [id]);
